...
import { type Category, TransactionType } from "#prisma";
...
export const initializeGlobalCategories = async (): Promise<Category[]> => {
  ...
  for (const categry of globalCategories) {
    try {
      ...
      if (!existing) {
        const newCatgory = await prisma.category.create({ data: categry });
        console.log(`✅ ${newCatgory.name}`);
        createdCategories.push(newCatgory);
      } else {
        createdCategories.push(existing);
      }
    } catch (err) {
      console.error("🚨 error ao criar categoria:", err);
    }
  }
  console.log("🚀 todas as categorias iniciadas");
